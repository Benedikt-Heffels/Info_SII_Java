...
public class minsortKorrigiert extends Frame {
    // Anfang Attribute
    ...
    int x, zweiY, paragraph; //"identifier" expected: § kein Variablenname,Variablennamen musst mit Buchstaben beginnen (nicht 2y)
    // Ende Attribute
    ...
    public minsort(String title) {
        ...
        button2.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
                button1_ActionPerformed(evt); //"button2_ActionPerformed" nicht vorhanden -> stattdessen: button1
            }
        });
    ...
    public static int[] MinSort(int[] r) { // r muss deklariert werden, nicht als Teil der Klammer (']' expected, identifier expected)
        ...
        int j, temp;
        while i < n {
            int min = i; //';' expected; min muss deklariert werden (als int: 'not a statement')
            j == i+1;
            while (j <= n) {
                if (r[j]<r[min])
                    Min = j;
